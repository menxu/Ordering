...
public class ImageCache {
    ...
    private static boolean cancel_download(String image_url, ImageView image_view) {
        ...
        if (task != null) {
            ...
            if (Utile.is_str_blank(task_image_url) || !task_image_url.equals(image_url)) {
                task.cancel(true);
            } else {
    ...
    // 规则是：
    // ->
    // /ording/image_cache/ uri.hashCode()+".cache"
    public static File get_cache_file(String image_url) {
        try {
            URI uri = new URI(image_url);
//            String filename = uri.hashCode() + ".cache";
            String filename = uri.hashCode() + ".png";
            File cache_file = new File(Utile.IMAGE_CACHE_DIR, filename);
